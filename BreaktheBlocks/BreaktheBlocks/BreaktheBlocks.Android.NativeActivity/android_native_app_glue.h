 ...
 * 애플리케이션 제공 콜백 집합을 기반으로 합니다.
 *
 * 이는 각각의 콜백 중 하나를 차단해서는 안 되고 
 * 시스템이 애플리케이션을 강제로 종료하는 위험이 있음을 의미합니다.
 * 이 프로그래밍 모델은 직접적이고 가볍지만 제한적입니다.
 *
 * 'threaded_native_app' 정적 라이브러리는 애플리케이션이 다른 스레드에서
 * 대신 주 이벤트 루프를 구현할 수 있는
 * 다른 실행 모델을 제공하는 데 사용됩니다. 사용 방법은 다음과 같습니다.
 *
 * 1/ 애플리케이션은 "android_main()"이라는 함수를 제공해야 합니다.
 * 이 함수는 활동의 주 스레드로부터
 *  고유한 새 스레드에서 활동을 만들었을 때 호출됩니다.
 *
 * 2/ android_main()은 애플리케이션이 실행되는
 * ANativeActivity 개체 인스턴스와 같이 다른 중요한 개체로의 참조를 포함하는
 ...
 * 해당합니다.
 *
 * 애플리케이션이 추가 파일 설명자를 수신하는 데 동일한 ALooper를
...
/**
 * 스레드 애플리케이션의 표준 붙이기 코드에 대한
 * 인터페이스입니다. 이 모델에서 애플리케이션의 코드는
 * 프로세스의 주 스레드에서 분리된 자체 스레드에서 실행됩니다.
 * 스레드가 Java VM과 연결될 필요는 없지만
 * JNI에서 Java 개체를 호출하게 하려면 연결해야 할 수
 * 있습니다.
 */
struct android_app {
    // 애플리케이션은 필요한 경우 여기에 상태 개체를 가리키는 포인터를
    // 배치할 수 있습니다.
    void* userData;
    ...
    // 애플리케이션의 NativeActivity가 제거되었거나
...
enum {
    ...
    /**
     * 애플리케이션 창의 AInputQueue에서 오는 이벤트의 Looper 데이터 ID로
