...
*
* Apache 라이선스 2.0 버전(이하 "라이선스")에 따라 사용 권한이 부여됩니다.
...
* 어떠한 명시적, 묵시적인 유형의 보증이나 조건 없이 배포됩니다.
* 라이선스에 따른 특정 언어의 관리 권한 및 제한에 대한 내용은
* 라이선스를 참조하세요.
*
*/
...
static int engine_init_display(struct engine* engine) {
	...
	* ANativeWindow 버퍼가 일치하도록 안전하게 다시 구성할 수 있습니다. */
	...
	/*for (int i = 0; i < 5 ; ++i)
	{
		renderer->setupRenderer(tempObject[i], CIRCLE);
	}*/
...
static int32_t engine_handle_input(struct android_app* app, AInputEvent* event) {
	...
	if (AInputEvent_getType(event) == AINPUT_EVENT_TYPE_MOTION) {
		...
		//state.x --> 터치된곳에서 x, y 위치 받아옴.
		sceneManager->input(actionType, engine->state.x, engine->state.y);
		return 1;
	}
	return 0;
}
